...
// Parent/student surfaces show "Name - Year" so classes with the same name
// across different year groups (or just for clarity) read the same way the
// teacher already sees them.
function classDisplayName(cls) {
  if(!cls?.name) return cls?.subject || '';
  return cls.year_group ? `${cls.name} - ${cls.year_group}` : cls.name;
}
